...
def resize_input(input_data: dict, tgt_size=224) -> dict:
    ...
    for key in input_data.keys():  # For each person
        for i in range(len(input_data[key])):  # For each image
            ...
            # Resize into 224x448
            resized_data[key][i] = np.array(
                Image.fromarray(resized_data[key][i]).resize(
                    (int(0.5 * tgt_size), tgt_size)
                )
            )
            ...
            pad = (
                np.abs(resized_data[key][i].shape[1] - resized_data[key][i].shape[0])
                // 2
            )
            resized_data[key][i] = np.pad(
                resized_data[key][i], ((0, 0), (pad, pad), (0, 0)), "constant"
            )
...
def train_test_split(
    ...
    X_train = np.array(
        [np.array([np.array(img) for img in person]) for person in X_train.values()]
    )
    X_test = np.array(
        [np.array([np.array(img) for img in person]) for person in X_test.values()]
    )
...
def read_raw():
    ...
    for person in ear_data:
        ear_imgs[person] = [
            cv2.cvtColor(
                cv2.imread("./data/AWE/%s/%02d.png" % (person, i)), cv2.COLOR_BGR2RGB
            )
            for i in range(1, 11)
        ]
