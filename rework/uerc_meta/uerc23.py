...
class UERC23():
    ...
    def __init__(self, images_path, annotations_csv, full_image_list_csv, data_splits_csv=None):
        self.device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        self.images_path = images_path
        self.annotations_csv = annotations_csv

        os.makedirs('runs', exist_ok=True)
        self.uerc_dataset = UERCDataset('test', self.images_path, self.annotations_csv, full_image_list_csv, data_splits_csv)

        if STORE_RESULTS:
            os.makedirs(RUN_PATH, exist_ok=True)

        print("Running on:", str(self.device).upper())
        print("Storing feature vectors:", FEATURES_PATH if STORE_FEATURE_VECTORS else "No")
        print("Drawing plots:", os.path.join(PLOTS_PATH, '[covariate]' + PLOT_FORMAT) if DRAW_AND_STORE_PLOTS else "No")
    ...
    def compute_feature_vectors(self, folder_list, covariate_name):
        ...
        # Load the dataset
        dataset = datasets.ImageFolder(self.images_path, transform=self.uerc_dataset.transforms['test'])

        # Filter the dataset to only include the desired subfolders:
        dataset.samples = [s for s in dataset.samples if s[0].split('/')[-2] in folder_list]
        dataset.targets = [s[1] for s in dataset.samples]



        dataloader = torch.utils.data.DataLoader(dataset, batch_size=1024, shuffle=False, num_workers=0)
        ...
        if STORE_FEATURE_VECTORS:
            ...
            np.savetxt(os.path.join(FEATURES_PATH, covariate_name + '.csv'), feature_vectors, delimiter=' ')

            # Save classes to a txt file
            np.savetxt(os.path.join(FEATURES_PATH, covariate_name + '.classes.csv'), classes, delimiter=' ', fmt='%d')
    ...
    def compute_performance(self, features, labels, covariate_name):
        ...
        for class_ in np.unique(labels):
            fpr_interp = np.interp(np.arange(n_points), np.arange(len(fpr[class_])), fpr[class_])
            tpr_interp = np.interp(np.arange(n_points), np.arange(len(tpr[class_])), tpr[class_])
            fpr_mean += fpr_interp
            tpr_mean += tpr_interp
        ...
        fnr = 1 - tpr
        min_index = np.argmin(np.abs(fpr - fnr)) # same as np.argmin(np.sqrt((fpr ** 2) + ((1 - tpr) ** 2)))
        eer = fpr[min_index]
        ...
        if DRAW_AND_STORE_PLOTS:
            ...
            plt.plot([eer], [1-eer], marker='o', markersize=5, label='EER = %0.2f' % eer)
            plt.xlim([0.0, 1.0])
            plt.ylim([0.0, 1.0])
    ...
    def compute_results_for_one_covariate(self, folder_list, covariate_name, precomputed_features_path):
        ...
        if precomputed_features_path is None:
            ...
            features, labels = self.load_feature_vectors(precomputed_features_path, covariate_name)


        # Now we can compute similarity matrix, and compute all the scores:
        roc_auc, eer, ver, r1 = self.compute_performance(features, labels, covariate_name)
    ...
    def compute_results_for_all(self, precomputed_features_path):
        ...
        for covariate, subject_list in selected_images.items():
            ...
            img_count, class_count, roc_auc, eer, ver, r1 = self.compute_results_for_one_covariate(subject_list, covariate_form, precomputed_features_path)
            scores[:, i] = roc_auc, eer, ver, r1

            s = covariate_form + "\t{}\t{}\t{:.2f}\t{:.2f}\t{:.2f}\t{:.2f}\n".format(img_count, class_count, roc_auc, eer, ver, r1)
            self.log(s)
            i += 1
        ...
        print("Means: AUC ({:.2f}%), EER ({:.2f}%), VER@1%FAR ({:.2f}%)\n".format(mean_auc, mean_eer, mean_ver))    

        elapsed_time = time.time() - start_time
        minutes = int(elapsed_time // 60)
        seconds = int(elapsed_time % 60)
        print(f"Elapsed time: {minutes}m {seconds}s.\n" if minutes > 0 else f"Elapsed time: {seconds}s.\n")
