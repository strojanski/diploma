...
class EarTriplet(Dataset):
    def __init__(self, anchor_data, anchor_labels, positive_data, positive_labels, negative_data, negative_labels):
    ...
    def __len__(self):
        return min(len(self.anchor_data), min(len(self.positive_data), len(self.negative_data)))
    
    def __getitem__(self, index):
        return (self.anchor_data[index], self.positive_data[index], self.negative_data[index]), (self.anchor_labels[index], self.positive_labels[index], self.negative_labels[index])
